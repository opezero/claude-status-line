...
Claude Code 用のカスタムステータスラインスクリプトです。ターミナル下部に、モデル・契約プラン・コンテキスト使用量・レートリミット・Git 状態・トークン消費履歴などを 2 行で表示します。Ghostty での利用を想定していますが、Nerd Fonts に対応したターミナルであれば動作します。

## 表示イメージ

![ステータスライン表示例](docs/statusline.png)

ステータスラインは 2 行構成で、1 行目にセッション情報（モデル・プラン・コンテキストなど）、2 行目に履歴・Git・レートリミットを表示します。
...
| レートリミット（5 時間枠） | 使用率＋リセットまでの残り時間 |
| レートリミット（7 日枠） | 使用率＋リセットまでの残り時間 |
